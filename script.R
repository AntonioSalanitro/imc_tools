...
convert_to_vector <- function(input_string) {
  ...
  markers <- strsplit(input_string, "\\|")[[1]]
  
  return(markers)
}
result <- convert_to_vector(ch_to_exclude)
print(result)
cur_snr <- lapply(images_comp, function(img){
...
cur_snr_filtered <- cur_snr %>% filter(!marker %in% result)
...
ggsave(filename = "Signal_intensity_filtered.pdf", plot = p, width = 10, height = 10)
